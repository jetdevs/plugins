...
# PostToolUse hook: After create-specs or codex-review completes, launch codex
# - create-specs → codex runs /spec-feedback-reviewer to review the new specs
# - codex-review → codex reviews Claude's inline responses to GPT5's feedback in feedback.md
...
elif [[ "$skill_name" == "codex-review" || "$skill_name" == *":codex-review" ]]; then
  is_codex_review=true
fi

# Exit if neither skill matched
if [[ "$is_create_specs" == false && "$is_codex_review" == false ]]; then
  exit 0
fi
...
elif [[ "$is_codex_review" == true ]]; then
...
# Launch codex in background with workspace-write so it can create feedback.md
nohup codex exec --sandbox workspace-write "$codex_prompt" > /tmp/codex-spec-review-latest.log 2>&1 &
